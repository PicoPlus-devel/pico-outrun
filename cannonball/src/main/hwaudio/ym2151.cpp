...
/* ---------------------------------------------------------------------------
 * picoOutRun: the seven lookup tables below are ~70 KB of .bss, which an RP2350
 * cannot spare (see port/README.md). They depend only on the chip clock
 * (OSoundInt::SOUND_CLOCK, 4 MHz) and the sample rate (44100) - both fixed at
 * build time - and nothing writes to them outside init_tables() and
 * init_chip_tables(). So hosttest/lutgen_ym2151 runs THIS FILE's own table code
 * on the host and emits them as const arrays that live in flash.
 *
 * The #defines below redirect every use to the generated tables; the
 * definitions and the two init bodies are compiled out. Any stray write would
 * become a compile error against const, which is the point.
 * ------------------------------------------------------------------------- */
#ifdef OUTRUN_YM_LUT_IN_FLASH
#include "ym2151_luts.h"
/* Pointer aliases, NOT #defines: YM2151Operator has its own `freq` member, and
 * a macro would rewrite `op->freq` too. These fold to the flash addresses. */
static const uint32_t     *const tim_A_tab = ym2151_lut_tim_A_tab;
static const uint32_t     *const tim_B_tab = ym2151_lut_tim_B_tab;
static const uint32_t     *const freq      = ym2151_lut_freq;
static const int32_t      *const dt1_freq  = ym2151_lut_dt1_freq;
static const uint32_t     *const noise_tab = ym2151_lut_noise_tab;
static const int32_t      *const tl_tab    = ym2151_lut_tl_tab;
static const uint32_t     *const sin_tab   = ym2151_lut_sin_tab;
#endif
#ifndef OUTRUN_YM_LUT_IN_FLASH
...
{
    /* picoOutRun: tables come from flash; nothing to build. */
}
#else
void YM2151::init_tables()
{
...
#ifdef OUTRUN_LUTGEN
/* picoOutRun: hosttest/lutgen_ym2151 dumps the real tables through these, so
 * the generated header is byte-identical to what this file computes. */
const signed int  *outrun_ym2151_tl_tab(void)  { return tl_tab; }
const unsigned int*outrun_ym2151_sin_tab(void) { return sin_tab; }
int outrun_ym2151_tl_tab_len(void)  { return TL_TAB_LEN; }
int outrun_ym2151_sin_tab_len(void) { return SIN_LEN; }
#endif
