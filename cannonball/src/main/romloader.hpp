...
/* picoOutRun: the header uses uint8_t/uint16_t/uint32_t but did not include
 * this. Upstream got away with it because romloader.cpp included stdint.hpp
 * first; port/romloader.cpp does not. */
#include "stdint.hpp"
...
public:
    ...
    /* picoOutRun: point at a region of the flash data image without copying it.
     * See port/romloader.cpp. `owned` says whether unload() should free rom. */
    void set_flash(const uint8_t* data, uint32_t len);
    bool owned;
