...
void HWRoad::init(const uint8_t* src_road, const bool hires)
{
    ...
    roads = src_road; // already decoded by tools/mkoutrundata
#else
    if (src_road)
...
void OUTRUN_HOT(HWRoad::render_background_lores)(uint16_t* pixels)
{
    int x, y;
...
void OUTRUN_HOT(HWRoad::render_foreground_lores)(uint16_t* pixels)
{
    int x, y;
    uint16_t* roadram = ramBuff;
    ...
    for (y = 0; y < S16_HEIGHT; y++) 
    {
        ...
        const uint8_t *src0, *src1; /* picoOutRun: const, see hwtiles.cpp */
...
void HWRoad::render_foreground_hires(uint16_t* pixels)
{
    ...
    for (y = 0; y < config.s16_height; y++) 
    {
        ...
        const uint8_t *src0 = NULL, *src1 = NULL; /* picoOutRun: const */
