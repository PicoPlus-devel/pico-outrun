...
void hwtiles::init(const uint32_t* src_tiles, const bool hires)
{
    tiles = src_tiles; // already decoded by tools/mkoutrundata
#else
...
#endif // patch_tiles/restore_tiles: never reached at 320x224
...
void OUTRUN_HOT(hwtiles::update_tile_values)()
{
    for (int i = 0; i < 4; i++)
    {
...
void OUTRUN_HOT(hwtiles::render_tile_layer)(uint16_t* buf, uint8_t page_index, uint8_t priority_draw)
{
...
void OUTRUN_HOT(hwtiles::render_text_layer)(uint16_t* buf, uint8_t priority_draw)
{
    ...
    uint16_t nPaletteOffset) 
{
    ...
    /* picoOutRun: const so this compiles against either a flash pointer or the
     * decoded RAM array. Valid both ways, so no #ifdef needed. */
    const uint32_t* pTileData = tiles + (nTileNumber << 3);
